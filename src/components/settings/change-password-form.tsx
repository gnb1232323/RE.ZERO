...
export function ChangePasswordForm() {
  ...
  return (
    <form action={action} className="card-hover max-w-md space-y-4 rounded-2xl border border-ink-200 bg-white p-5 shadow-card sm:p-6">
      ...
      <div>
        ...
        {state?.errors?.currentPassword && <p className="mt-1 text-sm text-danger-600">{state.errors.currentPassword[0]}</p>}
      </div>
      ...
      <div>
        ...
        {state?.errors?.confirmPassword && <p className="mt-1 text-sm text-danger-600">{state.errors.confirmPassword[0]}</p>}
      </div>
